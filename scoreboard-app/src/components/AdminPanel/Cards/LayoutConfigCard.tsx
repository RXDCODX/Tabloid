...
const fields: Array<{ key: 'top' | 'left' | 'right' | 'width' | 'height'; label: string; placeholder: string }> = [
...
const LayoutConfigCard: React.FC<Props> = ({ layoutConfig, onChange }) => {
  ...
  return (
    <Card className={`mb-4 bg-dark text-white border-primary border-2 p-3 ${styles.layoutConfigCard}`}>
      <div className="d-flex flex-column align-items-center gap-2 text-center mb-2">
        <span className="fw-bold text-uppercase" style={{ color: '#6f42c1', letterSpacing: 1 }}>Layout</span>
      </div>
      <Card.Body>
        <div className="d-flex align-items-center justify-content-between mb-2">
          <small className="text-secondary">px и % до 100</small>
        </div>
            ...
            <Row className="g-2 justify-content-center">
              {fields
                ...
                .map(({ key: fkey, label, placeholder }) => (
                  <Col xs={12} md={6} lg={4} key={fkey} className="d-flex justify-content-center">
                    <Form.Group controlId={`${String(key)}-${fkey}`} className={`w-100 text-center ${styles.fieldBox} ${styles[fkey]}`}>
                      <Form.Label className="mb-1 text-white text-center">{label}</Form.Label>
                      <Form.Control
                        type="text"
